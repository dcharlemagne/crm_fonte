...
{
    public class RepListaDesconto<T> : CrmServiceRepository<T>, IListaDesconto<T>
    {
        ...
        public T ObterPor(Guid discounttypeid)
        {
            var query = GetQueryExpression<T>(true);
        ...
        public List<T> ListarPor(String nomeListaDesconto)
        {
            var query = GetQueryExpression<T>(true);

            #region Condições
            Microsoft.Xrm.Sdk.Query.ConditionExpression cond1 = new Microsoft.Xrm.Sdk.Query.ConditionExpression("name", Microsoft.Xrm.Sdk.Query.ConditionOperator.Equal, nomeListaDesconto);
            query.Criteria.Conditions.Add(cond1);
            #endregion
