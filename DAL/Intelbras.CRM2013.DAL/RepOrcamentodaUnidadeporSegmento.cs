...
{
    public class RepOrcamentodaUnidadeporSegmento<T> : CrmServiceRepository<T>, IOrcamentodaUnidadeporSegmento<T>
    {
        ...
        public List<T> ListarSegmentodoOrcamento(Guid orcamentoId)
        {
            var query = GetQueryExpression<T>(true);

            #region Condições
            query.Criteria.AddCondition("statecode", ConditionOperator.Equal, 0);
            query.AddLink("itbc_orcamentoportrimestredaunidade", "itbc_orcamentoportrimestredaunidadeid", "itbc_orcamentoportrimestredaunidadeid").AddLink(
                "itbc_orcamentodaunidade", "new_orcamentoporunidadeid", "itbc_orcamentodaunidadeid").LinkCriteria.AddCondition("itbc_orcamentodaunidadeid", ConditionOperator.Equal, orcamentoId);
            #endregion
        ...
        public T ObterOrcamentoSegmento(Guid segmentoId, Guid trimestreId)
        {
            var query = GetQueryExpression<T>(true);
        ...
        public T ObterOrcamentoSegmento(Guid unidadenegocioId, int ano, int trimestre, Guid segmentoId, params string[] colunas)
        {
            var query = GetQueryExpression<T>(true);

            if (colunas != null && colunas.Length > 0)
            {
                query.ColumnSet.AddColumns(colunas);
            }
        ...
        public List<T> ListarSegmentodoOrcamentoproTrimestre(Guid trimestreId)
        {
            var query = GetQueryExpression<T>(true);
        ...
        public List<T> ListarOrcamentoSegmento(int ano, int trimestre)
        {
            var query = GetQueryExpression<T>(true);
        ...
        /********************      ACESSO SQL      ********************/
        public DataTable ListarOrcamentoSegmentoDW(int ano, int trimestre, List<OrcamentodaUnidade> lstOrcamentodaUnidade)
        {
