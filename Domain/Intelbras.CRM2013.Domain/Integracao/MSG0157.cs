...
{
    public class MSG0157 : Base, IBase<Message.Helper.MSG0157, Domain.Model.SolicitacaoBeneficio>
    {
        //ShowRoom

        public MSG0157(string org, bool isOffline) : base(org, isOffline) { }
        ...
        public string Executar(string mensagem, string numeroMensagem, Usuario usuario)
        {
            ...
            usuarioIntegracao = usuario;
                var objPollux = CarregarMensagem<Pollux.MSG0157>(mensagem);
                var objeto = DefinirPropriedades(objPollux);
            if (!resultadoPersistencia.Sucesso)
            {
                retorno.Add("Resultado", resultadoPersistencia);
                return CriarMensagemRetorno<Pollux.MSG0157R1>(numeroMensagem, retorno);
            }
        ...
        private void AtualizarProdutos(Pollux.MSG0157 objPollux, Guid? solicitacaoID)
                {
            string origem = "";
        ...
        public SolicitacaoBeneficio DefinirPropriedades(Intelbras.Message.Helper.MSG0157 xml)
        {
        ...
        private Intelbras.Message.Helper.MSG0157 DefinirPropriedadesPlugin(SolicitacaoBeneficio crm)
        {
            Intelbras.Message.Helper.MSG0157 objPollux = new Pollux.MSG0157(Domain.Enum.Sistemas.RetornaSistema(Domain.Enum.Sistemas.Sistema.CRM), crm.Nome.Truncate(40));
        ...
        {
            string resposta;
            Intelbras.Message.Helper.MSG0157 mensagem = DefinirPropriedadesPlugin(objModel);

            Domain.Servicos.Integracao integracao = new Servicos.Integracao(this.Organizacao, this.IsOffline);
            if (integracao.EnviarMensagemBarramento(mensagem.GenerateMessage(true), "1", "1", out resposta))
            {
                Intelbras.Message.Helper.MSG0157R1 retorno = CarregarMensagem<Pollux.MSG0157R1>(resposta);
