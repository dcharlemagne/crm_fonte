...
{
    public class MSG0154 : Base, IBase<Message.Helper.MSG0154, Domain.Model.SolicitacaoBeneficio>
    {
        //Rebate
        #region Construtor

        public MSG0154(string org, bool isOffline) : base(org, isOffline) { }
        ...
        public string Executar(string mensagem, string numeroMensagem, Usuario usuario)
        {
                ...
                var objPollux = CarregarMensagem<Pollux.MSG0154>(mensagem);
                var objeto = DefinirPropriedades(objPollux);

                if (!resultadoPersistencia.Sucesso)
                    {
                        retorno.Add("Resultado", resultadoPersistencia);
                        return CriarMensagemRetorno<Pollux.MSG0154R1>(numeroMensagem, retorno);
                    }
        ...
        public SolicitacaoBeneficio DefinirPropriedades(Intelbras.Message.Helper.MSG0154 xml)
        {
            ...
            String CodigoEmitente = String.Empty;

            if (!String.IsNullOrEmpty(xml.CodigoConta))
            {
                Conta conta = new Servicos.ContaService(this.Organizacao, this.IsOffline).BuscaConta(Guid.Parse(xml.CodigoConta));
                if (conta == null)
                {
                    resultadoPersistencia.Sucesso = false;
                    resultadoPersistencia.Mensagem = "Conta: " + xml.CodigoConta + " não encontrado no Crm.";
                    return crm;
                }
                else
                    CodigoEmitente = conta.CodigoMatriz;
        ...
        public Intelbras.Message.Helper.MSG0154 DefinirPropriedadesPlugin(SolicitacaoBeneficio crm)
        {
            Intelbras.Message.Helper.MSG0154 objPollux = new Pollux.MSG0154(Domain.Enum.Sistemas.RetornaSistema(Domain.Enum.Sistemas.Sistema.CRM), Helper.Truncate(crm.Nome, 40));
            ...
            objPollux.ProdutoSolicitacaoItens = RetornaListaItens(crm.ID.Value);
            objPollux.SolicitacaoAjuste = crm.AjusteSaldo;
            objPollux.ValorAbater = (crm.ValorAbater.HasValue) ? crm.ValorAbater.Value : 0m;
        ...
        {
            string resposta;
            Intelbras.Message.Helper.MSG0154 mensagem = DefinirPropriedadesPlugin(objModel);

            Domain.Servicos.Integracao integracao = new Servicos.Integracao(this.Organizacao, this.IsOffline);

            if (integracao.EnviarMensagemBarramento(mensagem.GenerateMessage(true), "1", "1", out resposta))
            {
                Intelbras.Message.Helper.MSG0154R1 retorno = CarregarMensagem<Pollux.MSG0154R1>(resposta);
                ...
                else
                {
                    if (retorno.SaldoBeneficioCanal != null)
                    {
                        ...
                        new BeneficioDoCanalService(Organizacao, IsOffline).AlterarBeneficioCanal(beneficioCanal);
                    }
                }
            }
            else
