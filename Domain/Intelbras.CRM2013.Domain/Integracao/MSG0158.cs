...
{
    public class MSG0158 : Base, IBase<Message.Helper.MSG0158, Domain.Model.SolicitacaoBeneficio>
    {
        public MSG0158(string org, bool isOffline) : base(org, isOffline) { }
        ...
        public string Executar(string mensagem, string numeroMensagem, Usuario usuario)
        {
            ...
            usuarioIntegracao = usuario;
                var objPollux = CarregarMensagem<Pollux.MSG0158>(mensagem);
                var objeto = DefinirPropriedades(objPollux);

            if (!resultadoPersistencia.Sucesso)
            {
                retorno.Add("Resultado", resultadoPersistencia);
                return CriarMensagemRetorno<Pollux.MSG0158R1>(numeroMensagem, retorno);
            }
        ...
        private void AtualizarProdutos(Pollux.MSG0158 objPollux, Guid? solicitacaoID)
            {
            string origem = "";
            ...
            List<ProdutosdaSolicitacao> lstProdutoSolicitacao = this.DefinirPropriedadesItens(objPollux.ProdutoSolicitacaoItens, origem, solicitacaoID);
        ...
        public SolicitacaoBeneficio DefinirPropriedades(Intelbras.Message.Helper.MSG0158 xml)
        {
        ...
        private Intelbras.Message.Helper.MSG0158 DefinirPropriedadesPlugin(SolicitacaoBeneficio crm)
        {
            Intelbras.Message.Helper.MSG0158 objPollux = new Pollux.MSG0158(Domain.Enum.Sistemas.RetornaSistema(Domain.Enum.Sistemas.Sistema.CRM), crm.Nome.Truncate(40));
            ...
            if (crm.BeneficioPrograma != null)
            {
                ...
                Beneficio beneficio = new Servicos.BeneficioService(this.Organizacao, false).ObterPor(crm.BeneficioPrograma.Id);

                if (beneficio != null && beneficio.Codigo.HasValue)
                {
                    objPollux.CodigoBeneficio = beneficio.ID.Value.ToString();
                    objPollux.BeneficioCodigo = beneficio.Codigo.Value;
                }
                else
                {
                    throw new ArgumentException("(CRM) BeneficioPrograma não preenchido ou codigo(int) do BeneficioPrograma não preenchido.");
                }
            }
            else
            {
        ...
        {
            string resposta;
            Pollux.MSG0158 mensagem = DefinirPropriedadesPlugin(objModel);

            Domain.Servicos.Integracao integracao = new Servicos.Integracao(this.Organizacao, this.IsOffline);
            if (integracao.EnviarMensagemBarramento(mensagem.GenerateMessage(true), "1", "1", out resposta))
            {
                Pollux.MSG0158R1 retorno = CarregarMensagem<Pollux.MSG0158R1>(resposta);

                if (!retorno.Resultado.Sucesso)
                {
                    throw new ApplicationException(retorno.Resultado.Mensagem);
                }
                else
                {
                    if (retorno.SaldoBeneficioCanal != null)
                    {
                        BeneficioDoCanal beneficioCanal = new BeneficioDoCanalService(Organizacao, IsOffline).ObterPor(new Guid(retorno.SaldoBeneficioCanal.CodigoBeneficioCanal));
