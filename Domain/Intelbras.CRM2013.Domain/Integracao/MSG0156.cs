...
{
    public class MSG0156 : Base, IBase<Message.Helper.MSG0156, Domain.Model.SolicitacaoBeneficio>
    {
        //Stockrotation
        public MSG0156(string org, bool isOffline) : base(org, isOffline) { }
            ...
            try
        {
            usuarioIntegracao = usuario;
                var objPollux = CarregarMensagem<Pollux.MSG0156>(mensagem);
                var objeto = DefinirPropriedades(objPollux);
            if (!resultadoPersistencia.Sucesso)
            {
                retorno.Add("Resultado", resultadoPersistencia);
                return CriarMensagemRetorno<Pollux.MSG0156R1>(numeroMensagem, retorno);
            }
                ...
                new TarefaService(this.Organizacao, this.IsOffline).AtualizarTarefaNaoAjusteManual(objeto);
                //new SolicitacaoBeneficioService(this.Organizacao, this.IsOffline).Avanca(objeto);
        ...
        public SolicitacaoBeneficio DefinirPropriedades(Intelbras.Message.Helper.MSG0156 xml)
        {
        ...
        private List<ProdutosdaSolicitacao> DefinirPropriedadesItens(List<Pollux.Entities.ProdutoSolicitacaoItem> listSolicitacaoItens, string origem, Guid? solicitacaoID)
        {
            ...
            foreach (var itemPollux in listSolicitacaoItens.Where(x => x.Situacao == (int)Enum.ProdutoSolicitacao.Status.Ativo))
            {
                ...
                //if (!String.IsNullOrEmpty(itemPollux.CodigoSolicitacaoBeneficio) && itemPollux.CodigoSolicitacaoBeneficio.Length == 36)
                //{
                //    produtoSolicitacao.SolicitacaoBeneficio = new Lookup(new Guid(itemPollux.CodigoSolicitacaoBeneficio), "");
                //}
        ...
        private Intelbras.Message.Helper.MSG0156 DefinirPropriedadesPlugin(SolicitacaoBeneficio crm)
        {
            Intelbras.Message.Helper.MSG0156 objPollux = new Pollux.MSG0156(Domain.Enum.Sistemas.RetornaSistema(Domain.Enum.Sistemas.Sistema.CRM), crm.Nome.Truncate(40));
        ...
        {
            string resposta;
            Intelbras.Message.Helper.MSG0156 mensagem = DefinirPropriedadesPlugin(objModel);

            Domain.Servicos.Integracao integracao = new Servicos.Integracao(this.Organizacao, this.IsOffline);
            if (integracao.EnviarMensagemBarramento(mensagem.GenerateMessage(true), "1", "1", out resposta))
            {
                Intelbras.Message.Helper.MSG0156R1 retorno = CarregarMensagem<Pollux.MSG0156R1>(resposta);
