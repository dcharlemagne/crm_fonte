...
{
    public class MSG0152 : Base, IBase<Message.Helper.MSG0152, Domain.Model.SolicitacaoBeneficio>
    {
        //VMC
        public MSG0152(string org, bool isOffline) : base(org, isOffline) { }
        ...
        public string Executar(string mensagem, string numeroMensagem, Domain.Model.Usuario usuario)
        {
            try
            {
                ...
                var objPollux = CarregarMensagem<Pollux.MSG0152>(mensagem);
                var objeto = DefinirPropriedades(objPollux);

                if (!resultadoPersistencia.Sucesso)
                {
                    retorno.Add("Resultado", resultadoPersistencia);
                    return CriarMensagemRetorno<Pollux.MSG0152R1>(numeroMensagem, retorno);
                }
                ...
                List<ProdutosdaSolicitacao> lstProdutoSolicitacao = DefinirPropriedadesItens(objPollux.ProdutoSolicitacaoItens, objeto.ID);
                objeto.ID = new SolicitacaoBeneficioService(Organizacao, IsOffline).PersistirMensagemIntegracao(objeto, lstProdutoSolicitacao);

                RelacionaUnidadesNegocio(this.CarregarMensagem<Pollux.MSG0152>(mensagem), objeto);
        ...
        public SolicitacaoBeneficio DefinirPropriedades(Intelbras.Message.Helper.MSG0152 xml)
        {
            ...
            if (!string.IsNullOrEmpty(xml.IdentidadeEmissor))
                crm.IntegradoDe = xml.IdentidadeEmissor;
            else
                crm.AddNullProperty("IntegradoDe");
            ...
            if (xml.ValorAcao.HasValue)
                crm.ValorAcao = xml.ValorAcao;
            else
                crm.AddNullProperty("ValorAcao");
            ...
            if (!String.IsNullOrEmpty(xml.CodigoAcaoSubsidiadaVMC) && xml.CodigoAcaoSubsidiadaVMC.Length == 36)
            {
                AcaoSubsidiadaVmc acao = new Servicos.AcaoSubsidiadaVmcService(this.Organizacao, this.IsOffline).ObterPor(new Guid(xml.CodigoAcaoSubsidiadaVMC));
                if (acao != null)
                    crm.AcaoSubsidiadaVmc = new Lookup(acao.ID.Value, "");
                else
                {
                    resultadoPersistencia.Sucesso = false;
                    resultadoPersistencia.Mensagem = "CodigoAcaoSubsidiadaVMC : " + xml.CodigoAcaoSubsidiadaVMC + " - não cadastrado no Crm.";
                    return crm;
                }
            }
            else
            {
                resultadoPersistencia.Sucesso = false;
                resultadoPersistencia.Mensagem = "CodigoAcaoSubsidiadaVMC não Enviado ou fora do padrão(Guid).";
                return crm;
            }
            ...
            if (xml.DataInicioAcao.HasValue)
                crm.DataIniAcao = xml.DataInicioAcao;
            else
                crm.AddNullProperty("DataIniAcao");

            if (xml.DataPrevistaRetornoAcao.HasValue)
                crm.DataFimAcao = xml.DataPrevistaRetornoAcao;
            else
                crm.AddNullProperty("DataFimAcao");
            ...
            if (xml.ResultadoPrevisto.HasValue)
                crm.ResultadoPrevisto = xml.ResultadoPrevisto.Value;

            if (xml.ResultadoAlcancado.HasValue)
                crm.ResultadoAlcancado = xml.ResultadoAlcancado.Value;
        ...
        private List<ProdutosdaSolicitacao> DefinirPropriedadesItens(List<Pollux.Entities.ProdutoSolicitacaoItem> listSolicitacaoItens, Guid? solicitacaoID)
        {
            ...
            foreach (var itemPollux in listSolicitacaoItens.Where(x => x.Situacao == (int)Enum.ProdutoSolicitacao.Status.Ativo))
            {
                ...
                //Passar o Id da solicitação da mensagem
        ...
        {
            string resposta;
            Intelbras.Message.Helper.MSG0152 mensagem = DefinirPropriedadesPlugin(objModel);

            Domain.Servicos.Integracao integracao = new Servicos.Integracao(this.Organizacao, this.IsOffline);
            if (integracao.EnviarMensagemBarramento(mensagem.GenerateMessage(true), "1", "1", out resposta))
            {
                Intelbras.Message.Helper.MSG0152R1 retorno = CarregarMensagem<Pollux.MSG0152R1>(resposta);
