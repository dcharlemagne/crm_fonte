...
function ValidarClienteIsol(clienteId) {
    /*** EXECUTA O WEBSERVICE ***/
    Util.funcao.CallServiceXML(Config.ParametroGlobal.IntegrationWS.Vendas.Isolservice, "PesquisaClientePor");
    Util.funcao.SetParameter("clienteId", clienteId);
    Util.funcao.SetParameter("organizacao", Xrm.Page.context.getOrgUniqueName());
    //Execução do serviço web
    var retorno = Util.funcao.Execute();
...
function AtualizaArvoreAssunto(id) {
    ...
    if (UN_DO_USUARIO != "ISOL" && UN_DO_USUARIO != "ENGENHARIA APLICAÇÃO") {
...
function AoSalvar(contexto) {
    ...
    if (Xrm.Page.ui.getFormType() == 2)
        ...
        if (UN_DO_USUARIO == "ISOL" || UN_DO_USUARIO == "Intelbras" || UN_DO_USUARIO == "ENGENHARIA APLICAÇÃO") {
...
function AoCarregar() {
    ...
    **********************************/
    ...
    try {
        ...
        //Xrm.Page.getControl("new_emitente").setVisible(Xrm.Page.getAttribute("new_emitente").getValue() != null ? true : false); - Robson 08/12/2017

        //Desabilita o número de série quando o usuário for o CRMADMIN
        //Retirado por Robson - 08/12/2017
        //if (FormularioEstaSendoAlterado == true && Xrm.Page.getAttribute("createdby").getValue()[0].name.indexOf("admin") > 0)
        //    Xrm.Page.getControl("productserialnumber").setDisabled(true);

        //Chama Função para Listar  Range de Valores para o Picklist
        switch (UN_DO_USUARIO) {

            case "ISOL":
            case "ENGENHARIA APLICAÇÃO":
                //Obriga campo "empresa executante"
                ...
                inicio[0] = 200000;
                inicio[1] = 993520006; // Bradescos

                var fim = new Array();
                fim[0] = 200006;
                fim[1] = 993520010; // Bradesco

                ArrayRangePickList(Xrm.Page.getControl("statuscode"), Xrm.Page.getAttribute("statuscode"), inicio, fim);
                //RangePickList(Xrm.Page.getControl("new_causa_final"), Xrm.Page.getAttribute("new_causa_final"), 1, 10); //Causa Final

                Xrm.Page.getControl("new_acaoid").setVisible(false);
                Xrm.Page.getControl("new_acao2id").setVisible(false);
                Xrm.Page.getAttribute("new_acaoid").setRequiredLevel("none");
                Xrm.Page.getControl("new_qualificacao_cliente").setVisible(false);
                Xrm.Page.getControl("kbarticleid").setVisible(false);
                //habilita campo Reincidente
                Xrm.Page.getControl("new_reincidenteid").setDisabled(false);

                Xrm.Page.ui.tabs.get("tab_10").setVisible(false);
                Xrm.Page.ui.tabs.get("ADDITIONALDETAILS_TAB").setVisible(false);

                break;

            case "POS VENDA":

                RangePickList(Xrm.Page.getControl("casetypecode"), Xrm.Page.getAttribute("casetypecode"), 300000, 300019); //Tipo de Ocorrencia
                RangePickList(Xrm.Page.getControl("prioritycode"), Xrm.Page.getAttribute("prioritycode"), 200000, 200004); //Prioridade       
                ...
                Xrm.Page.getAttribute("new_acaoid").setRequiredLevel("required");

                //Esconde o campo new_usuario_conclusao_ocorrencia
                Xrm.Page.getControl("new_usuario_conclusao_ocorrencia").setVisible(false);

                //Desabilita as abas Contrato,Detalhe,Servicos e SLA 
            ...
            default:
                //Esconde o campo new_usuario_conclusao_ocorrencia
                Xrm.Page.getControl("new_usuario_conclusao_ocorrencia").setVisible(false);
                RangePickList(Xrm.Page.getControl("prioritycode"), Xrm.Page.getAttribute("prioritycode"), 1, 3);
...
function clienteParticipanteEnderecoOnChange() {
    if (Xrm.Page.getAttribute("itbc_cliente_participante_endereco").getValue() != null) {
        ...
        if (typeof (result.attributes['new_cep']) != 'undefined') {
            Xrm.Page.getAttribute("new_rua").setValue(result.attributes['new_rua'].value + " - " + result.attributes['new_cep'].value);
        } else {
...
function hideFieldsISOL() {
    Xrm.Page.getControl("caseorigincode").setVisible(false);
    Xrm.Page.getControl("new_produto_para_uso").setVisible(false);
    Xrm.Page.getControl("new_reincidenteid").setVisible(false);
    Xrm.Page.getControl("contractid").setVisible(false);
    Xrm.Page.getControl("contractdetailid").setVisible(false);
    Xrm.Page.getControl("new_empresa_executanteid").setVisible(false);
    Xrm.Page.getControl("new_tecnico_responsavelid").setVisible(false);
    Xrm.Page.getControl("new_data_origem").setVisible(false);
    Xrm.Page.getControl("new_data_hora_prevista_visita").setVisible(false);
    Xrm.Page.getControl("new_kilometragem_percorrida").setVisible(false);
    Xrm.Page.getControl("new_os_cliente").setVisible(false);
    Xrm.Page.getControl("new_designacao_cliente").setVisible(false);
    Xrm.Page.getControl("new_solicitanteid").setVisible(false);
}
